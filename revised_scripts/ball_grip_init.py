...
def init_biped(portHandler, part, pos):
	...
	groupSyncWrite1 = GroupSyncWrite(portHandler, packetHandler[t], ADDR_MX_GOAL_VELOCITY[t], 2)
    ...
    for dxl_index in range(NUM_DXLS[t]):  
        ...
        if dxl_addparam_result != 1:
            ...
            quit()
		# Allocate goal position value into byte array
        param_goal_velocity = [DXL_LOBYTE(DXL_LOWORD(50)), DXL_HIBYTE(DXL_LOWORD(50)), DXL_LOBYTE(DXL_HIWORD(50)), DXL_HIBYTE(DXL_HIWORD(50))]
        dxl_addparam_result = groupSyncWrite1.addParam(DXL[dxl_index].ID, param_goal_velocity)
        if dxl_addparam_result != 1:
            print("[ID:%03d] groupSyncWrite addparam failed" % (DXL[dxl_index].ID))
            quit() 

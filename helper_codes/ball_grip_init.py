...
ADDR_AX_MOVING                = 46
ADDR_AX_MOVING_VELOCITY       = 32

LEN_AX_MOVING_VELOCITY        = 2
...
f = open("../helper_codes/basket_grip.txt")
num_dxls_torso = 0
for line in f:
    num_dxls_torso += 1
f.close()
...
def grip(portHandler):
    ...
    for dxl_index in range(0, num_dxls_torso):
        dxl_comm_result, dxl_error = packetHandler1.write2ByteTxRx(portHandler, DXL_TORSO[dxl_index].ID, ADDR_AX_MOVING_VELOCITY, 100)       
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler1.getTxRxResult(dxl_comm_result), DXL_TORSO[dxl_index].ID)
        elif dxl_error != 0:
            print("%s" % packetHandler1.getRxPacketError(dxl_error), DXL_TORSO[dxl_index].ID)
